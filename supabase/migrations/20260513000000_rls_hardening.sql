...
-- All migrations consolidated as of 2026-05-13.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS "public"."sessions" (
    ...
    "state"                  "text" DEFAULT 'waiting'::"text" NOT NULL,  -- waiting | active | finished
    "current_question_index" integer,                                    -- nullable; index into questions array
    "question_open"          boolean DEFAULT true NOT NULL,              -- controls answer submission window
    "question_opened_at"     timestamp with time zone,                   -- set by trigger; used to compute response_time_ms
    "current_question_slots" "jsonb",                                    -- slot assignments broadcast to players via realtime
    "host_secret"            "uuid" DEFAULT "gen_random_uuid"() NOT NULL, -- hidden from client roles; passed to host RPCs
    "created_at"             timestamp with time zone DEFAULT "now"() NOT NULL
);
ALTER TABLE "public"."sessions" OWNER TO "postgres";

-- Maps each answer to a display slot (colour + icon) for a specific session+question.
-- Created by start_game / open_next_question; immutable thereafter.
CREATE TABLE IF NOT EXISTS "public"."session_question_answers" (
    "id"          "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "session_id"  "uuid" NOT NULL,
    "question_id" "uuid" NOT NULL,
    "slot_index"  integer NOT NULL,    -- 0-3; display position
    "answer_id"   "uuid" NOT NULL,
    "color"       "text" NOT NULL,     -- red | blue | yellow | green
    "icon"        "text" NOT NULL,     -- circle | diamond | triangle | square
...
CREATE TABLE IF NOT EXISTS "public"."players" (
    ...
    "secret"        "uuid" DEFAULT "gen_random_uuid"() NOT NULL,         -- hidden from client roles; passed to submit_answer
    "joined_at"     timestamp with time zone DEFAULT "now"() NOT NULL
);
ALTER TABLE "public"."players" OWNER TO "postgres";

-- One row per player per question.
-- points_earned defaults to 0 and is populated by close_question, not submit_answer.
CREATE TABLE IF NOT EXISTS "public"."player_answers" (
    "id"               "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "player_id"        "uuid" NOT NULL,
    "question_id"      "uuid" NOT NULL,
    "answer_id"        "uuid" NOT NULL,
    "points_earned"    integer DEFAULT 0 NOT NULL,         -- server-computed by close_question
    "response_time_ms" integer,                            -- ms from question_opened_at to submission
    "created_at"       timestamp with time zone DEFAULT "now"() NOT NULL
);
...
CREATE TABLE IF NOT EXISTS "public"."profiles" (
    ...
    "stripe_cancel_at_period_end" boolean DEFAULT false NOT NULL  -- UI hint: cancellation scheduled
);
...
-- Internal helper: shuffles the answers for a given question into 4 display slots,
-- writes them to session_question_answers, and returns the slot array as JSONB.
-- Called by start_game and open_next_question; not exposed directly to clients.
...
-- Trigger function: stamps sessions.question_opened_at whenever the question index changes
-- or the answer window reopens (question_open: false → true).
-- Used by submit_answer to compute response_time_ms server-side.
...
-- Auth hook: inserts a profiles row for every new auth.users entry.
-- Invoked via the on_auth_user_created trigger (defined in the auth section below).
...
-- Event trigger function: automatically enables RLS on any new table created in the
-- public schema, so no table can accidentally be left unprotected.
...
-- Quiz management RPCs
-- Creates a new quiz with all its questions and answers. Returns the new quiz UUID.
...
-- Atomically updates an existing quiz: patches metadata, deletes removed questions/answers,
-- and upserts remaining ones by ID.
...
-- Returns {session_id, join_code, host_secret}; the host_secret must be stored
...
-- Transitions a session from 'waiting' to 'active' and assigns answer slots for
-- the first question. Returns the slot assignment JSONB.
...
-- Advances to a new question: assigns fresh answer slots, updates
-- current_question_index, and reopens the answer window.
-- question_opened_at is stamped automatically by the sessions trigger.
...
-- Closes the answer window and scores all submitted answers.
-- Scoring: time-decayed points (50–100 % of face value based on response speed)
-- plus a streak bonus (+10 % per flame above 2 consecutive correct answers).
-- Wrong answers score 0. Updates player_answers.points_earned and players.score/streak/correct_count.
...
-- Returns the correct answer ID for a question, but only after the answer window
-- is closed. Prevents clients from reading the answer while the question is live.
...
-- Validates: player secret, answer window open, question matches the current one,
-- and the answer slot is valid for this session. Computes response_time_ms server-side
-- from sessions.question_opened_at. points_earned is left at 0 and set by close_question.
...
-- Stamps sessions.question_opened_at on question change or window reopen.
CREATE OR REPLACE TRIGGER "sessions_question_opened_at_trigger"
...
-- quizzes: creator-scoped writes; public quizzes readable by everyone
...
-- questions: visible when the parent quiz is public, owned, or part of an active session
...
CREATE POLICY "questions_insert_auth" ON "public"."questions"
    FOR INSERT WITH CHECK ((("auth"."uid"() IS NOT NULL) AND (EXISTS ( SELECT 1
       FROM "public"."quizzes"
...
-- answers: same visibility rules as parent quiz
...
-- sessions: all readable (state is public); writes only via security-definer RPCs
...
-- session_question_answers: read-only for all; written only via RPCs
...
-- players: readable for all; joined via join_session RPC
ALTER TABLE "public"."players" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "players_select_open" ON "public"."players"
    FOR SELECT USING (true);


-- player_answers: readable for all; written via submit_answer RPC
ALTER TABLE "public"."player_answers" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "player_answers_select" ON "public"."player_answers"
    FOR SELECT USING (true);
...
-- starred_quizzes: owner-scoped
ALTER TABLE "public"."starred_quizzes" ENABLE ROW LEVEL SECURITY;

CREATE POLICY "owner select" ON "public"."starred_quizzes"
    FOR SELECT USING (("auth"."uid"() = "user_id"));

CREATE POLICY "owner insert" ON "public"."starred_quizzes"
    FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));

CREATE POLICY "owner delete" ON "public"."starred_quizzes"
...
-- images bucket: upload/update/delete restricted to Pro users (profiles.is_pro = true),
-- scoped to their own folder ({userId}/...). All authenticated users can read own images.
CREATE POLICY "Pro users can upload images" ON "storage"."objects"
    FOR INSERT TO "authenticated"
    WITH CHECK ((("bucket_id" = 'images'::"text") AND (("storage"."foldername"("name"))[1] = ("auth"."uid"())::"text") AND (EXISTS ( SELECT 1
       FROM "public"."profiles"
      WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."is_pro" = true))))));

CREATE POLICY "Pro users can update own images" ON "storage"."objects"
    FOR UPDATE TO "authenticated"
    USING ((("bucket_id" = 'images'::"text") AND (("storage"."foldername"("name"))[1] = ("auth"."uid"())::"text") AND (EXISTS ( SELECT 1
       FROM "public"."profiles"
      WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."is_pro" = true))))))
...
CREATE POLICY "Users can read own images" ON "storage"."objects"
    ...
    USING ((("bucket_id" = 'images'::"text") AND (("storage"."foldername"("name"))[1] = ("auth"."uid"())::"text")));
...
-- Player RPCs: callable by anon (player_secret acts as the auth token)
...
-- NOTE: pg_dump does not capture cron.job rows; these must be re-declared here
-- so that a fresh db reset / new environment gets the jobs too.
-- -----------------------------------------------------------------------------
...
-- pg_dump sets search_path = '' for safety, but we need to restore it so that
-- seed.sql (which uses unqualified table names) can resolve public.* tables.
RESET search_path;
