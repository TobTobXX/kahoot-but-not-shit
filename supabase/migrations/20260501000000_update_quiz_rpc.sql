-- update_quiz: replaces all questions and answers for an existing quiz in one RPC call.
-- Ownership is enforced via RLS on the underlying tables (no SECURITY DEFINER needed).
-- p_questions is a JSONB array of:
--   { id, order_index, question_text, time_limit, points, image_url,
--     answers: [{ id, order_index, answer_text, is_correct }] }
...
BEGIN
  UPDATE public.quizzes
     SET title = p_title, is_public = p_is_public
   WHERE id = p_quiz_id;
